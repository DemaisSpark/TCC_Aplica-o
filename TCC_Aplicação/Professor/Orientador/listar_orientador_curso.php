...
$result_orientador = "SELECT * FROM orientador WHERE orientador = '$gerenciador' ORDER BY orientador_id DESC LIMIT $inicio, $qnt_result_pg";
...
<!-- Inicia a tabela que sera usada para exiir os conteudos do banco de dados selecionados pelo comando de $resultado_orientador  -->
	<thead>
		<tr>
			...
			<th>Login Curso</th>
			<!-- Cria a coluna Login Curso na tabela -->
			<th>Instituição</th>
			<!-- Cria a coluna Instituição na tabela -->
			<th>Status Vagas</th>
			<!-- Cria a coluna Status Vagas na tabela -->
		</tr>
	</thead>
	<tbody>
		<?php
		...
		// Instrui o codigo a pegar o conteudo das linhas do banco de dados, seguindo o comando de $resultado_orientador
			...
			<tr>
				...
				<td><?php if($row_orientador['status_vagas'] == 'aberto') {
						echo '<span style="color:#008700;">ABERTO</span>';
					} else {
						echo '<span style="color:#ff0303;">FECHADO</span>';
					}?>
				</td>
				...
				<td>
					...
					<button type="button" class="btn btn-xs btn-warning" data-toggle="modal" data-target="#exampleModal" data-whatever="<?php echo $row_orientador['orientador_id']; ?>" data-whatevernome="<?php echo $row_orientador['nome_orientador']; ?>"data-whateverdetalhes="<?php echo $row_orientador['temas']; ?>">Editar</button>
					...
					<a href="apagar_orientador_curso.php?id=<?php echo $row_orientador['orientador_id']; ?>"><button type="button" class="btn btn-xs btn-danger">Rejeitar</button></a>
					<!-- Estabelece o funcionamento do botão Apagar, enviando o valor de orientador_id, que e atrebuido a variavel id e enviado para a pagina apagar_orientador_orientador.php e redireciona a execução para o codigo contido nela -->
				</td>
			</tr>
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<p>Contato Orientador: <?php echo $row_orientador['contato_orientador']; ?></p>
							<!-- Apresenta os dados contidos na coluna contato_orientador -->
							...
							<p>Status Vagas: <?php echo $row_orientador['status_vagas']; ?></p>
							<!-- Apresenta os dados contidos na coluna status -->
							<p>Data Criação: <?php echo $row_orientador['data_orientador']; ?></p>
							<!-- Apresenta os dados contidos na coluna instituição_curso -->
							<p>Temas: <?php echo $row_orientador['temas']; ?></p>
							<!-- Apresenta os dados contidos na coluna instituição_curso -->
						</div>
					</div>
				</div>
...
echo "<span class='page-link'><a href='#' onclick='listar_orientador_orientador(1, $qnt_result_pg)'>Primeira</a></span>";
// Cria o link da pagina inicial, retornando a função listar_orientador_orientador(1, $qnt_result_pg) quando clicado, aparecendo como a palavra Primeira
echo '</li>';
...
// Imprime as duas paginas anteriores, que estão apos a primeira pagina, por meio da operação $pagina - $max_links, que corresponde a $pagina - 1 que sofre incrementações de acordo com as paginas
	...
	// Impede que o codigo imprima paginas menor que 1, ou seja 0 e -1
		echo "<li class='page-item'><a href='#' onclick='listar_orientador_orientador($pag_ant, $qnt_result_pg)'>$pag_ant </a></li>";
		// Cria o link da pagina inicial, retornando a função listar_orientador_orientador($pag_ant, $qnt_result_pg) quando clicado
	}
}
...
// Imprime as duas paginas posteriores, que estão apos a pagina atual, por meio da operação $pagina + $max_links, que corresponde a $pagina +max_links que sofre incrementações de acordo com as paginas
	...
	// Impede que o codigo imprima paginas maiores que o numero maximo de paginas
		echo "<li class='page-item'><a href='#' onclick='listar_orientador_orientador($pag_dep, $qnt_result_pg)'>$pag_dep</a></li>";
		// Cria o link da pagina final, retornando a função listar_orientador_orientador($pag_ant, $qnt_result_pg) quando clicado
	}
}

echo '<li class="page-item">';
echo "<span class='page-link'><a href='#' onclick='listar_orientador_orientador($quantidade_pg, $qnt_result_pg)'>Última</a></span>";
// Cria o link da pagina final, retornando a função listar_orientador_orientador($quantidade_pg, $qnt_result_pg) quando clicado, aparecendo como a palavra Última
echo '</li>';
echo '</ul>';
echo '</nav>';

}else{
	echo "<div class='alert alert-danger' role='alert'>Nenhuma orientador encontrada para este usuário!</div>";
