...
$curso = mysqli_real_escape_string($conexao, $user);
// Conecta $curso a variavel $user e a conexão estabelecida com o BD por meio de $conexao

$result_orientador = "SELECT * FROM orientador WHERE curso_login = '$curso' AND status_vagas = 'aberto' ORDER BY orientador_id DESC LIMIT $inicio, $qnt_result_pg";
	...
	<tbody>
		...
		// Instrui o codigo a pegar o conteudo das linhas do banco de dados, seguindo o comando de $resultado_orientador
			...
			<tr>
				...
				<td><?php echo $row_orientador['contato_orientador']; ?></td>
				<!-- Chama os valores correspondentes a orientador -->
				<td><?php echo $row_orientador['instituição_orientador']; ?></td>
				<!-- Chama os valores correspondentes a orientador -->
				<td><?if($row_orientador['status_vagas'] == 'aberto') {
				...
				<td>
				    <button type="button" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#myModal<?php echo $row_orientador['orientador_id']; ?>">Temas</button>
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<!-- Apresenta os dados contidos na coluna temas -->
						</div>
					</div>
				</div>
			</div>
...
echo '</nav>';
	...
	echo "<div class='alert alert-danger' role='alert'>Não há nenhum orientador com vagas abertas para este Curso!</div>";
